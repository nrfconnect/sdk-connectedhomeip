...
from zap_common import (DEFAULT_APP_TEMPLATES_RELATIVE_PATH, DEFAULT_ZAP_GENERATE_RELATIVE_PATH, DEFAULT_ZAP_VERSION_RELATIVE_PATH,
                        DEFAULT_ZCL_JSON_RELATIVE_PATH, ZapInstaller, find_zap,
...
class TestWestZap(unittest.TestCase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.test_dir = MATTER_BASE / "test_dir"
        self.test_zap_file = self.test_dir / "test.zap"
        self.zcl_json_file = self.test_dir / "zcl.json"
        self.zcl_json_file_with_new_items = self.test_dir / "zcl_test.json"
        self.test_clusters = [SCRIPT_DIR / "Cluster1.xml", SCRIPT_DIR / "Cluster2.xml"]
        self.app_templates = APP_TEMPLATES_FILE
        self.version_file = self.test_dir / 'scripts' / 'setup' / 'zap.version'
        self.recommended_version = ''
        self.zap_output_dir = self.test_dir / "zap-generated"

        # Prepare the test directory
        if not self.test_dir.exists():
            self.test_dir.mkdir(parents=True, exist_ok=True)

        self.zcl_json_file.touch()
        self.zcl_json_file_with_new_items.touch()

        # Ensure destination directories exist before copying
        self.version_file.parent.mkdir(parents=True, exist_ok=True)

        shutil.copy(BASE_ZCL_FILE, self.zcl_json_file)
        shutil.copy(TEST_ZCL_FILE, self.zcl_json_file_with_new_items)
        shutil.copy(TEST_ZAP_FILE, self.test_zap_file)
        shutil.copy(VERSION_FILE, self.version_file)

        with open(self.version_file, 'r') as f:
            self.recommended_version = f.read().strip()
    ...
    def test_synchronize_zcl_with_base(self):
        """
        Checks whether the synchronize_zcl_with_base function synchronizes the zcl.json file with the base zcl.json file.

        We expect:
            - The missing "Window Covering" cluster is added to the zcl.json file.
        """
        with open(self.zcl_json_file_with_new_items, 'r') as f:
            target_data = json.load(f)
            self.assertFalse("window-covering.xml" in target_data["xmlFile"])

        synchronize_zcl_with_base(self.zcl_json_file_with_new_items, MATTER_BASE)

        with open(self.zcl_json_file_with_new_items, 'r') as f:
            updated_data = json.load(f)
            # xmlRoot should be skipped
            self.assertEqual(updated_data["xmlRoot"], [".", "./data-model/chip"])
            # other elements should be updated
            xmlFile = updated_data["xmlFile"]
            self.assertIn("window-covering.xml", xmlFile)
            attributeAccessInterfaceAttributes = updated_data["attributeAccessInterfaceAttributes"]
            self.assertIn("Window Covering", attributeAccessInterfaceAttributes)
            self.assertIn("ClusterRevision", attributeAccessInterfaceAttributes["Window Covering"])
    ...
    def test_append_cluster(self):
        ...
        with open(self.zcl_json_file, 'r') as f:
            ...
            self.assertIn("attr3", output_data["attributeAccessInterfaceAttributes"]["Cluster 2"])
...
def suite():
    ...
    test_order = [
        ...
        'test_synchronize_zcl_with_base',
