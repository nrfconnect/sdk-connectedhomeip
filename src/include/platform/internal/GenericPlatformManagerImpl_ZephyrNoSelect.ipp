...
#ifndef GENERIC_PLATFORM_MANAGER_IMPL_ZEPHYR_NO_SELECT_CPP
#define GENERIC_PLATFORM_MANAGER_IMPL_ZEPHYR_NO_SELECT_CPP
...
CHIP_ERROR GenericPlatformManagerImpl_Zephyr<ImplClass>::_StopEventLoopTask(void)
{
    ...
    // If the loop is stopped by a thread other than the event loop thread,
    // trigger the event loop thread to recalculate its wait time by posting a no-op event
    // to the event queue.
    if (k_current_get() != &mChipThread)
    {
...
#endif // GENERIC_PLATFORM_MANAGER_IMPL_ZEPHYR_NO_SELECT_CPP
